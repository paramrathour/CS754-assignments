...
for video = videos
    ...
    for t = T
        RMSE_info = process(video, t, dimensions, patch_size, seed, threshold, RMSE_info, start_heights, start_widths);
    end
end
...
function RMSE_info = process(image_name, T, dimensions, patch_size, seed, threshold, RMSE_info, start_heights, start_widths)
    rng(seed);
    ...
    % RMSE_info = RMSE_info + "RMSE = " + string(RMSE) + " for " + image_name + "reconstructed, T = " + newline;
end
...
function image_reconstructed = patch_reconstruct(coded_snapshot, patch_size, code, dct_matrix, threshold)
    ...
    for i = 1:number_of_rows+1-patch_size
        for j = 1:number_of_columns+1-patch_size
            ...
            for k=1:T
                phi{k} = diag(c(:,k));
                % phi{i} = spdiags(c(:,i), 0, H*W, H*W); % use sparse matrices for large patches
            end
            ...
            number_of_overlapping_patches(i:i+patch_size-1, j:j+patch_size-1, :) = number_of_overlapping_patches(i:i+patch_size-1, j:j+patch_size-1, :) + ones(patch_size, patch_size, T);
        end
    end
