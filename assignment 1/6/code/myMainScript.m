...
k_special = [5 50 200];
rng(0)
process(m, m_special, k, k_special, dimensions);

function [] = process(m, m_special, k, k_special, dimensions)
    ...
    for i = 1:length(k)
        ...
        for j = 1:length(m)
            ...
            image_reconstructed = omp(dimensions, phi*image_vector, k(i), phi, DCT_matrix);
            filename = "../images/" + "k = " + string(k(i)) + ", m = " + string(m(j)) + ".png";
